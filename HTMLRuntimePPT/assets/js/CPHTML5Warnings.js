var RuntimeBrowserDetect = {
	...
	init: function(iWarning, iTitle,iBtnText)
	{
		...
		if (!this['BrowserEnum'])
		{
			...
			this.BrowserEnum.FF_MIN_SUPPORTED_VERSION_WIN = 22;
			this.BrowserEnum.FF_MIN_SUPPORTED_VERSION_MAC = 35;
			this.BrowserEnum.FF_MIN_SUPPORTED_VERSION_FFOS = 15;
			this.BrowserEnum.FF_MIN_SUPPORTED_VERSION_ANDROID = 20;
		}
		if (!this['DeviceEnum'])
		{
		...
		else if(navigator.userAgent.match(/Trident/i))
		{
			...
			{
				this.browser = this.BrowserEnum.MSIE;
				this.browserVersion = this.getCurrentBrowserVersion(navigator.userAgent,"rv") || this.BrowserEnum.UNKNOWN;				
			}
		}
	...
	getIsBrowserSupported: function()
	{
		...
		if((this.browser == this.BrowserEnum.MSIE) && (this.browserVersion >= this.BrowserEnum.MSIE_MIN_SUPPORTED_VERSION ))
			...
			lSupported = true;	
		else if((this.browser == this.BrowserEnum.FIREFOX) && 
					(((cp.OS == cp.MACOS) && (this.browserVersion >= this.BrowserEnum.FF_MIN_SUPPORTED_VERSION_MAC )) ||
						((cp.OS == cp.WINDOWS) && (this.browserVersion >= this.BrowserEnum.FF_MIN_SUPPORTED_VERSION_WIN ))))
			lSupported = true;	
		return lSupported;
	},
	...
	showRuntimeWarning: function(lMessageStr, lTitleStr, lButtonStr)
	{
		...
		var lButtonDivElementCH = function(){ var lWarningBoxElement = document.getElementById("CPUnSupportedBrowserWarning_ID"); if(lWarningBoxElement!=undefined){document.body.removeChild(lWarningBoxElement);}};
		cp.registerGestureEvent(lButtonDivElement,cp.GESTURE_EVENT_TYPES.TAP,lButtonDivElementCH);
